...
const Viewer = ({ dziUrl, filename }) => {
  ...
  // Helper to determine if the current image is a patch
  const isPatch = selectedImage && (selectedImage.endsWith('.png.dzi') || selectedImage.endsWith('.png'));
  ...
  const fetchAvailableModels = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/available_models`);
  ...
  const fetchNormalizedAnnotations = async (bounds, filename) => {
    ...
    try {
      addBlur();
      setNotification('Loading Annotations...'); // Set loading before fetch
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_MONGODB_URL}/get_normalized_annotations`,
        { bounds, filename: actualFilename, zoom: currentZoom, modelName: selectedModel }
      );
      const data = response.data;
      if (!data || Object.keys(data).length === 0) {
        setAnnotationsByFile({});
        setNotification(`No annotations found for model '${selectedModel}'.`);
        removeBlur(); // Remove blur if no data
        return {};
      }
  ...
  const drawAnnotationsWithPixi = () => {
    ...
    const currentZoom = viewer.viewport.getZoom();
    // Only apply zoom threshold for WSIs
    if (!isPatch && currentZoom <= 7) {
          ...
          geometry.coordinates.forEach((polygon, polygonIndex) => {
            ...
            polygon.forEach((ring, ringIndex) => {
              ...
              ring.forEach(([x, y], pointIndex) => {
                // Apply offset adjustment for patches in image coordinates
                let xOffset = x;
                let yOffset = y;
                if (isPatch) {
                  xOffset += 32;
                ...
                const viewportPoint = viewer.viewport.imageToViewportCoordinates(xOffset, yOffset);
                const screenPoint = viewer.viewport.viewportToViewerElementCoordinates(viewportPoint);
  ...
  const handlePanZoomEnd = async () => {
    ...
    if (!isPatch && currentZoom <= 7) {
      // Only for WSIs: show hex bins at low zoom
      pixiAppRef.current.stage.removeChildren();
      setAnnotations([]);
      renderHexBins();
    } else {
      // For patches (at any zoom) and WSIs (zoom > 7): show annotations
  ...
  const handleAnnotationUpload = async (file) => {
    ...
    formData.append('modelName', selectedModel); // Add model name
    try {
      ...
      if (error.response && error.response.status === 409) {
        alert('Annotation for this image and model already exists.');
      } else {
  ...
  useEffect(() => {
    // Only fetch if both image and model are selected
    if (selectedImage && selectedModel && viewer) {
      const bounds = getViewportBounds();
      if (bounds) {
        fetchNormalizedAnnotations(bounds, selectedImage);
      }
    }
  ...
  return (
    <div className="viewer-container">
      <div className="viewer-header">
        <h1></h1> {/* TODO: Whole Slide Image Viewer */}
      </div>
      <div className="dropdown-container">
        <label style={{ marginRight: '8px' }}>Select an available image:</label>
        <select onChange={handleImageChange} value={selectedImage} style={{ marginRight: '24px', minWidth: '220px' }}>
        ...
        <label style={{ marginRight: '8px' }}>Select model:</label>
        <select onChange={handleModelChange} value={selectedModel} style={{ minWidth: '160px' }}>
      ...
      <div className="viewer-wrapper">
        <div className="viewer-box">
          ...
          <div className="annotation-legend">
            <ul>
              {zoomValue <= 7 ? (
                ...
                <ul>
                  {Object.entries(annotationsByFile).some(([_, annotationGroup]) => annotationGroup.length > 0) ? (
                    ...
                    <li style={{ color: '#888', fontStyle: 'italic' }}>No annotation types to display</li>
                  )}
                </ul>
              )}
            </ul>
          ...
          <div className="annotation-toggles">
            ...
            {notification && (
              <div>
                <p className={notification.includes('No annotations found') ? 'error-status' : 'current-status'}>
                  <strong>Current Status:</strong> {notification}
                </p>
              </div>
            )}
            {Object.entries(annotationsByFile).map(([filename, annotationGroup]) => (
      ...
      <h2 style={{ color: '#0d3862ff', fontWeight: 600 }}>Link annotations to selected DZI</h2>
      <div className="upload-section">
        <input type="file" onChange={handleAnnotationFileChange} accept=".json,.geojson" multiple />
        <button onClick={handleMultipleAnnotationUpload} className="upload-btn" disabled={!selectedModel}>
          Upload annotation
        </button>
      </div>
